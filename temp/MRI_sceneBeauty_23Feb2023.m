...
if cfg.atBION == 1
    ...
    mainDir = [ ...
    '/Users/philippflieger/PhD/projects/' ...
    'fusion-study_scene-attractiveness' ...
    ];

    cd([mainDir, '/code', '/experiment']);
    addpath([mainDir, '/stimuli']);
    addpath([mainDir, '/sourcedata']);
end
...
cfg.screenSize = [win.width, win.height]./2;  % get resolution (`./2` Mac-specific)
...
% experiment info
dat.expName = 'fusion_scene-attractiveness';
dat.date = datetime('today');
dat.run = -1;  % purposefully invalid run number

% make sure no runs get overwritten
incompleteInfo = 1;
testRun = 0;

while incompleteInfo == 1
    % participant ID
    while true
        promptSubjID = input('Enter subject number: ');
    
        % ensure valid participant ID is given
        if ~isempty(promptSubjID)
            dat.subjID = promptSubjID;
    
            % convert subject ID to string for file naming
            if dat.subjID < 10
                strSubjID = ['0', num2str(dat.subjID)];
            else
                strSubjID = num2str(dat.subjID);
            end

            break;
        else
            fprintf('Subject ID cannot be empty. Please try again.\n\n');
        end
    end

    % run number
    while true
        promptRun = input('Enter run number: ');

        % ensure valid run number is entered
        if ~isempty(promptRun)
            dat.run = promptRun;
            ...
            if dat.run == 99
                ...
                % ensure correct run index for file naming
                strRun = num2str(dat.run);
    
                % create `test` dir. if it doesn't exist yet
                testDir = [mainDir, '/sourcedata/test'];

                if ~exist(testDir, 'dir')
                    mkdir(testDir);
                end
            else
                strRun = ['0', num2str(dat.run)];
            end
    ...
    if testRun ~= 1
        filename = sprintf( ...
            [mainDir, '/sourcedata', '/sub-%s_run%s_%s_%s.mat'], ...
            strSubjID, ...
            strRun, ...
            dat.expName, ...
            dat.date ...
    ...
    abortedFilename = fullfile(path, [name, '_ABORTED', ext]);
    if exist(abortedFilename, 'file'); filename = abortedFilename; end
    ...
    if ~exist(filename, 'file')
        ...
        if strcmpi(overwritePrompt, 'y')
            ...
            if correctedRunID ~= dat.run
                ...
                strRun = ['0', num2str(dat.run)];  % add 0-prefix to corr. run ID
                filename = sprintf( ...
                    [mainDir, '/sourcedata', '/sub-%s_run%s_%s_%s.mat'], ...
                    strSubjID, ...
                    strRun, ...
...
%{
    If this doesn't work the way it does on my laptop in the MRI, comment
    out the while-loop and simply run:

    ```
    dat.expName = 'fusion_scene-attractiveness';
    dat.date = datetime('today');
    dat.subjID = input('Enter subject ID: ');
...
for trial = 1:cfg.numStims
    ...
    while respComplete == 0
        ...
        elseif keyCode(cfg.abortKey) == 1  % abort experiment
            ...
            [path, name, ext] = fileparts(filename);
            filename = fullfile(path, [name, '_ABORTED', ext]);
            save(filename, 'dat');
            respComplete = 1;
            sca;
        end
