...
for iVox = 1:length(cfg.numVoxels)
    ...
    for category = cfg.categories
        ...
        if strcmp(cfg.searchlightSource, 'HCP')
            ...
            d.HCP_ISC.(category).runwiseVecRDMs = nan(nPairs, nParcels, numel(runSample));
        elseif strcmp(cfg.searchlightSource, 'allROI')
            nParcels = length(cfg.rois_of_interest); 
            d.allROI_ISC.(category).runwiseVecRDMs = nan(nPairs, nParcels, numel(runSample));
        end
        ...
        for iRun = 1:numel(runSample)
            ...
            for parc = 1:size(sub_table,2)
                ...
                if strcmp(cfg.searchlightSource, 'HCP')
                    d.HCP_ISC.(category).runwiseVecRDMs(:,parc,iRun) = squareform(R)';
                elseif strcmp(cfg.searchlightSource, 'allROI')
                    d.allROI_ISC.(category).runwiseVecRDMs(:,parc,iRun) = squareform(R)';
                end
        ...
        if cfg.saving
            ...
            if strcmp(cfg.searchlightSource, 'HCP')
                ...
                meanISC = mean(d.HCP_ISC.(category).runwiseVecRDMs, 3); % [pairs × parcels × runs]
                d.HCP_ISC.(category).mean = meanISC;

            elseif strcmp(cfg.searchlightSource, 'allROI')
                outFile = fullfile(cfg.outputPath,'group_level', ...
                    sprintf('ISC_allROI_%s_%d_voxels.mat', category, nVox));
                meanISC = mean(d.allROI_ISC.(category).runwiseVecRDMs, 3); % [pairs × parcels × runs]
                d.allROI_ISC.(category).mean = meanISC;
