...
for var = 1:numel(cfg.RDM_to_partial_out)
    ...
    elseif strcmp(cfg.RDM_to_partial_out{var}, 'photos_late')
        ...
        colors(var,:) = [.4, .9, 1];
    elseif strcmp(cfg.RDM_to_partial_out{var}, 'behResponses')
        short_names{var} = 'Behavioral Responses';
        colors(var,:) = [.4, 1, .3];
    elseif strcmp(cfg.RDM_to_partial_out{var}, 'betweenRDM')
        short_names{var} = 'betweenRDM';
        colors(var,:) = [.3, .3, .7];
    elseif strcmp(cfg.RDM_to_partial_out{var}, 'withinRDM')
        short_names{var} = 'withinRDM';
        colors(var,:) = [.3, .3, .1];
    end
end
% prepare figure
    ...
    if cfg.plotting
        ...
        elseif strcmp(cfg.plot_type, 'bar')
            for xiPos = 1:height(res_table)
                ...
                barHandles(current_x_pos) = bar(current_x_pos, res_table.r_val(xiPos),...
        ...
        for xiPos = 1:height(res_table)
            ...
            if cfg.permutation_test || cfg.bootstrapping
                ...
                elseif strcmp(cfg.plot_type, 'violin')
                    ...
                    plot([current_x_pos-0.1,current_x_pos+0.1],...
                        [res_table.r_val(xiPos), res_table.r_val(xiPos)], 'k', 'LineWidth',2);
                end
            end
